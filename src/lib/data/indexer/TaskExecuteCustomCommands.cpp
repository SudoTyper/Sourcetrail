	...
	std::shared_ptr<PersistentStorage> storage)
{
	if (indexerCommand)
	{
		const FilePath databaseFilePath = indexerCommand->getDatabaseFilePath();
		const FilePath databaseDirectoryPath = databaseFilePath.getParentDirectory();
		if (!databaseDirectoryPath.empty() && !databaseDirectoryPath.exists())
		{
			FileSystem::createDirectories(databaseDirectoryPath);
		}
